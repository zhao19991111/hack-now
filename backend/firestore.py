...
class firebaseAPI:
    ...
    def store(self, col, cat, data=[]):
        doc_ref = self.db.collection(
            u"{}".format(col)).document(u"{}".format(cat))
        i = 5
        for d in data:
...
db = firebaseAPI()
postcode = list(db.db.collection(u'users').document(
    u'postcode').get().to_dict().values())

for item in list_of_items:
    for postcode in postcode:
        for market in list_of_market:
            print("scraping {0} at {1} with postcode {2}".format(
